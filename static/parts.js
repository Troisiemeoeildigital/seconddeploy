...
const setMatheader = document.querySelectorAll('.setMatheader')
 ...
 const setupMaterialUI = (data) => {
    let html = '';
    data.forEach(doc=> {
      let  proof = `${doc.data().proofurl}`;
          if (proof.indexOf('https://firebasestorage.googleapis.com') > -1 ) {
             const li = `
...
<a href="#" class="navbary__link">
  ...
  <a href="#" class="navbary__link">
    ...
    <td>${doc.data().materialGroup}</td>
      ...
      <td ><a class="fileicon" >
 <i class="ri-close-circle-fill " style="font-size: 25px;  color: black;"></i>
</a></td>
...
<a href="#" class="navbary__link">
  ...
  setPMWeight.innerHTML = "Total Materials/Parts Weight: " + sumVal.toFixed(2) + " / " + partWeightRef;
      ...
      //   if (eachfileIcon.href == "https://kitech.onrender.com/partsupplier/parts/undefined" || "https://kitech.onrender.com/partsupplier/undefined" || "https://kitech.onrender.com/undefined") {
...
document.querySelector(".editfiles").addEventListener("change", function(e) {
  editfiles = e.target.editfiles; //it was files instead of editfiles
  for (let i = 0; i < editfiles.length; i++) {
    console.log(editfiles[i]);
    document.querySelector('.editprodImgphld').innerHTML = editfiles[i].name
  }
});
// continue here: check the edit file feature, it's not working
